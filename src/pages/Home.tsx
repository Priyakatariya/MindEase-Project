...
const SHOWCASE_CARDS = [
  { image: "https://images.unsplash.com/photo-1545205597-3d9d02c29597?auto=format&fit=crop&q=80&w=1200", label: "Mindful Breathing", title: "Find Peace in Every Breath", desc: "Simple guided breathing exercises to calm your mind and reduce anxiety within minutes." },
  { image: "https://images.unsplash.com/photo-1499209974431-9dddcece7f88?auto=format&fit=crop&q=80&w=1200", label: "Nature & Calm", title: "Nature Heals What Words Cannot", desc: "Reconnect with the world around you. Even five minutes of mindful awareness can shift your entire mood." },
  { image: "https://images.unsplash.com/photo-1523240795612-9a054b0db644?auto=format&fit=crop&q=80&w=1200", label: "Community Support", title: "You Are Never Alone Here", desc: "Thousands of students share your journey. Find your tribe, share your story, heal together." },
  { image: "https://images.unsplash.com/photo-1506126613408-eca07ce68773?auto=format&fit=crop&q=80&w=1200", label: "Morning Meditation", title: "Start Your Day With Intention", desc: "Morning mindfulness sessions to set a positive tone for the day ahead — even just 5 minutes." },
  { image: "https://images.unsplash.com/photo-1516062423079-7ca13cdc7f5a?auto=format&fit=crop&q=80&w=1200", label: "Safe Space", title: "A Safe Haven Built for Students", desc: "Every student deserves a place to breathe, reflect, and seek help without judgment." },
];
...
const QuoteCard = ({ image, quote, color }: { image: string; quote: string; color: string }) => (
  <div className="relative flex-shrink-0 rounded-[1.8rem] overflow-hidden cursor-pointer group"
...
const FeedbackMarquee: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      const docs = snap.docs.map(d => ({ id: d.id, ...d.data() })).filter((d: any) => d.type === 'feedback' && d.content);
      setItems(docs);
    });
    return () => unsub();
  }, []);
  ...
  const FeedCard = ({ v }: { v: any }) => (
    <div className="flex-shrink-0 rounded-[1.8rem] p-5 group relative overflow-hidden"
      style={{ width: 280, background: 'rgba(255,255,255,0.80)', border: '1.5px solid #F9C5CC', backdropFilter: 'blur(14px)', boxShadow: '0 4px 20px rgba(212,97,122,0.06)' }}>
      <div className="absolute bottom-0 left-0 right-0 h-0.5 opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: 'linear-gradient(90deg,#D4617A,#F4A0B0)' }} />
      <Heart size={14} className="fill-current mb-3 opacity-50" style={{ color: '#F4A0B0' }} />
      <p className="text-sm italic leading-relaxed mb-3 line-clamp-3" style={{ color: '#3D1520' }}>"{v.content}"</p>
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs font-black truncate" style={{ color: '#D4617A' }}>{v.nickname || v.anonymousName || 'Anonymous'}</p>
        <span className="text-[9px] font-black px-2 py-0.5 rounded-full shrink-0" style={{ background: '#FFE8ED', color: '#C44A6A' }}>
      ...
      <FadeIn>
        <div className="text-center mb-12 px-6">
          <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-3" style={{ color: '#3D1520' }}>
            What Our{' '}
            <span style={{ background: 'linear-gradient(135deg, #D4617A, #C44A6A)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Community Says</span>
          </h2>
          <p className="text-sm max-w-md mx-auto font-medium" style={{ color: '#7A3545', opacity: 0.75 }}>Anonymous voices from students, alumni & professors — shared with care.</p>
        </div>
      </FadeIn>
...
const Home: React.FC = () => {
  ...
  useEffect(() => { const t = setInterval(() => setHeroImgIdx(p => (p + 1) % HERO_IMAGES.length), 3000); return () => clearInterval(t); }, []);
  useEffect(() => { getCountFromServer(collection(db, 'users')).then(s => setLiveUsers(s.data().count)).catch(() => { }); }, []);
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'ratings'), snap => {
      ...
      if (vals.length) { const avg = vals.reduce((a, b) => a + b, 0) / vals.length; setLiveRating(avg.toFixed(1) + '★'); }
    }, () => { });
    return () => unsub();
  }, []);
  useEffect(() => { const t = setInterval(() => { setCarouselDir(1); setCarouselIdx((p) => (p + 1) % SHOWCASE_CARDS.length); }, 5000); return () => clearInterval(t); }, []);
  useEffect(() => { const t = setInterval(() => setMentorIdx((p) => (p + 1) % MENTORS.length), 4000); return () => clearInterval(t); }, []);

  const goCarousel = (dir: 1 | -1) => { setCarouselDir(dir); setCarouselIdx((p) => (p + dir + SHOWCASE_CARDS.length) % SHOWCASE_CARDS.length); };
  const goMentor = (dir: 1 | -1) => setMentorIdx((p) => (p + dir + MENTORS.length) % MENTORS.length);
  const mentorVisible = [(mentorIdx - 1 + MENTORS.length) % MENTORS.length, mentorIdx, (mentorIdx + 1) % MENTORS.length];
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden" style={{ background: 'rgba(244,241,255,0.18)' }}>
      ...
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
        <motion.div className="absolute rounded-full" style={{ width: 700, height: 700, top: '-15%', left: '-10%', background: 'radial-gradient(circle, rgba(249,197,204,0.45) 0%, transparent 70%)' }} animate={{ x: [0, 50, 0], y: [0, 35, 0] }} transition={{ duration: 20, repeat: Infinity, ease: 'easeInOut' }} />
        <motion.div className="absolute rounded-full" style={{ width: 600, height: 600, bottom: '-10%', right: '-5%', background: 'radial-gradient(circle, rgba(232,133,154,0.30) 0%, transparent 70%)' }} animate={{ x: [0, -40, 0], y: [0, -25, 0] }} transition={{ duration: 24, repeat: Infinity, ease: 'easeInOut' }} />
        <motion.div className="absolute rounded-full" style={{ width: 400, height: 400, top: '45%', left: '52%', background: 'radial-gradient(circle, rgba(255,220,225,0.35) 0%, transparent 70%)' }} animate={{ x: [0, 25, -18, 0], y: [0, -30, 18, 0] }} transition={{ duration: 28, repeat: Infinity, ease: 'easeInOut' }} />
      </div>
        ...
        <section className="relative min-h-[94vh] flex items-center px-6 pt-24 pb-16 overflow-hidden">
          {[
            ...
            <motion.div key={i} className="absolute rounded-full pointer-events-none"
              style={{ width: b.w, height: b.h, top: b.top, left: b.left, background: `radial-gradient(circle, ${b.color} 0%, transparent 68%)`, filter: 'blur(18px)' }}
              animate={{ x: b.xA, y: b.yA }} transition={{ duration: b.dur, repeat: Infinity, ease: 'easeInOut', delay: b.delay }} />
          ))}

          <div className="relative z-10 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.85, ease: [0.22, 1, 0.36, 1] }}>
              <motion.h1 initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.85, delay: 0.1, ease: [0.22, 1, 0.36, 1] }}
                className="text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] mb-4 tracking-tighter" style={{ color: '#3D1520' }}>
                Your Mind Deserves <br />
                <span style={{ background: 'linear-gradient(135deg, #D4617A 0%, #F472B6 50%, #BE185D 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                  To Be Seen, Heard & Healed
                </span>
              </motion.h1>
              <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.22 }}
                className="text-lg md:text-2xl font-light leading-relaxed mb-9 max-w-2xl italic" style={{ color: '#7A3545' }}>
              ...
              <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.34 }} className="flex flex-wrap gap-4 mb-10">
                <div className="relative">
                  <motion.div animate={{ scale: [1, 1.25, 1], opacity: [0.4, 0, 0.4] }} transition={{ duration: 2.2, repeat: Infinity, ease: 'easeInOut' }}
                    className="absolute inset-0 rounded-2xl pointer-events-none" style={{ background: 'rgba(212,97,122,0.35)', filter: 'blur(8px)' }} />
                  <button onClick={() => navigate('/sessions')}
                    ...
                    <Users size={20} /> Guided Sessions <ArrowRight size={16} />
                  </button>
                </div>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="flex gap-8">
                {[
                  { value: liveUsers !== null ? `${liveUsers.toLocaleString()}+` : '...', label: 'Registered Members', color: '#D4617A' },
                  ...
                  <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 + i * 0.12 }} className="text-center relative">
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, delay: 0.15, ease: [0.22, 1, 0.36, 1] }}
              ...
              <motion.div initial={{ opacity: 0, scale: 0.88, rotateY: -18 }} animate={{ opacity: 1, scale: 1, rotateY: 0 }}
                transition={{ duration: 1.1, delay: 0.3, ease: [0.22, 1, 0.36, 1] }} style={{ width: '86%', height: 430, perspective: '1000px' }} className="relative z-10">
                <div className="w-full h-full rounded-[2.5rem] overflow-hidden relative"
                  style={{ transform: 'rotateX(5deg) rotateY(-9deg)', transformStyle: 'preserve-3d', border: '3px solid rgba(255,255,255,0.90)', boxShadow: '0 50px 100px rgba(61,21,32,0.30), 0 20px 50px rgba(212,97,122,0.22), 16px 28px 70px rgba(0,0,0,0.18)' }}>
                  <AnimatePresence mode="sync">
                    <motion.img key={heroImgIdx} src={HERO_IMAGES[heroImgIdx]} alt="mental wellness" className="absolute inset-0 w-full h-full object-cover"
                      initial={{ opacity: 0, scale: 1.06 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.9 }} />
                  </AnimatePresence>
                  <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, rgba(61,21,32,0.20) 0%, transparent 50%, rgba(61,21,32,0.38) 100%)' }} />
                  <div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.16) 0%, transparent 44%)' }} />
                  <div className="absolute bottom-5 right-5 flex gap-2 z-10">
                    {HERO_IMAGES.map((_: string, i: number) => (
                      <button key={i} onClick={() => setHeroImgIdx(i)} className="rounded-full transition-all duration-300"
        ...
        <section className="px-6 py-24 bg-gradient-to-b from-[#FFF6F8] via-white to-white relative overflow-hidden">
          ...
          <FadeIn>
            <div className="text-center mb-16 relative z-10">
              <h2 className="text-4xl md:text-5xl font-black text-[#2E1A22] mb-5 tracking-tight">A Space Where <span className="text-[#D4617A]">You Truly Matter</span></h2>
              <p className="text-[#7A3545] max-w-xl mx-auto text-lg md:text-xl font-medium leading-relaxed opacity-80">
                Whether you're feeling overwhelmed, confused, unmotivated, or simply need clarity — these tools are here to support you gently, privately, and without judgment.
              </p>
            </div>
            <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
              {[
                { icon: <MessageCircle size={30} strokeWidth={2.5} />, title: "AI Support", desc: "Some days feel heavy. Some thoughts are hard to say out loud. Talk freely with our empathetic AI — available anytime you need comfort, clarity, or simply someone who listens.", link: "/chatbot", bg: "#FFE8ED", color: "#D4617A" },
                { icon: <BarChart3 size={30} strokeWidth={2.5} />, title: "Mood Tracker", desc: "Your emotions tell a story. Track how you feel each day, discover patterns over time, and gain gentle insights into what truly lifts you up and what weighs you down.", link: "/mood", bg: "#FFE0E6", color: "#C44A6A" },
                { icon: <Users size={30} strokeWidth={2.5} />, title: "Student Stories", desc: "You're not alone — even when it feels like you are. Read honest, anonymous journeys from fellow students, and share yours when you're ready to inspire someone else.", link: "/stories", bg: "#F8ECFF", color: "#E892A5" },
              ].map((card, i) => (
                <div key={i} className="group p-8 rounded-[2.5rem] bg-white border border-[#FFE8ED] shadow-[0_8px_30px_rgb(212,97,122,0.04)] hover:shadow-[0_20px_40px_rgb(212,97,122,0.1)] hover:-translate-y-2 transition-all duration-500">
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-7 group-hover:scale-110 transition-transform" style={{ background: card.bg, color: card.color }}>{card.icon}</div>
                  <h3 className="text-2xl font-bold text-[#2E1A22] mb-3">{card.title}</h3>
                  <p className="text-[#7A3545] leading-relaxed mb-6 font-medium opacity-90">{card.desc}</p>
                  <a href={card.link} className="inline-flex items-center font-bold text-[#C44A6A] gap-2 text-sm uppercase tracking-wider group-hover:gap-4 transition-all">
                    {card.title === "AI Support" ? "Start Talking" : card.title === "Mood Tracker" ? "See Your Journey" : "Discover Stories"} <ArrowRight size={18} />
                  </a>
                </div>
              ))}
            </div>
        ...
        <section className="py-20 overflow-hidden">
          <FadeIn>
            <div className="text-center mb-14 px-6">
              <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-4" style={{ color: '#3D1520' }}>
                Words That{' '}
                <span style={{ background: 'linear-gradient(135deg, #D4617A, #EC4899)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Restore You</span>
              </h2>
              <p className="text-base font-medium max-w-lg mx-auto" style={{ color: '#7A3545', opacity: 0.75 }}>Gentle reminders from people who've been there — because you deserve to know you're not broken, just human.</p>
            </div>
          </FadeIn>
          <div className="relative mb-5" style={{ maskImage: 'linear-gradient(90deg, transparent, black 12%, black 88%, transparent)' }}>
            <div className="flex gap-5 marquee-left" style={{ width: 'max-content' }}>
              {[...QUOTE_CARDS, ...QUOTE_CARDS].map((q, i) => <QuoteCard key={i} {...q} />)}
            </div>
          </div>
          <div className="relative" style={{ maskImage: 'linear-gradient(90deg, transparent, black 12%, black 88%, transparent)' }}>
            <div className="flex gap-5 marquee-right" style={{ width: 'max-content' }}>
              {[...QUOTE_CARDS_ALT, ...QUOTE_CARDS_ALT].map((q, i) => <QuoteCard key={i} {...q} />)}
            </div>
          </div>
        </section>
            ...
            <div className="max-w-6xl mx-auto">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-14 gap-8">
                <div>
                  <h2 className="text-4xl md:text-5xl font-black tracking-tight text-[#3D1520] leading-tight">
                    Your Wellness, <span className="bg-gradient-to-r from-[#D4617A] to-[#C44A6A] bg-clip-text text-transparent">Our Commitment</span>
                  </h2>
                  <p className="mt-4 text-lg md:text-xl text-[#7A3545] opacity-90 max-w-xl">Professional support, meaningful conversations, and safe spaces — thoughtfully designed to strengthen your emotional resilience.</p>
                </div>
                <button onClick={() => navigate('/sessions')} className="px-8 py-3 font-semibold rounded-2xl border border-[#F4A0B0] bg-white/70 backdrop-blur-md text-[#D4617A] hover:bg-[#D4617A] hover:text-white hover:shadow-lg hover:-translate-y-1 transition-all duration-300">View All Services</button>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {[
                  ...
                  <div key={i} onClick={() => navigate(service.link)}
                    className="group cursor-pointer p-6 rounded-2xl bg-white border border-[#FFE8ED] shadow-[0_8px_25px_rgba(212,97,122,0.05)] hover:shadow-[0_20px_50px_rgba(212,97,122,0.12)] hover:-translate-y-2 transition-all duration-400">
                    <div className="w-12 h-12 flex items-center justify-center rounded-xl mb-4 group-hover:scale-110 transition-transform duration-300" style={{ backgroundColor: service.accent + "20", color: service.accent }}>{service.icon}</div>
                    <h3 className="font-bold text-lg text-[#3D1520] group-hover:text-[#D4617A] transition-colors duration-300">{service.title}</h3>
                  </div>
                ))}
              </div>
            </div>
          ...
          <FadeIn>
            <div className="max-w-6xl mx-auto">
              <div className="text-center mb-14">
                <span className="inline-flex items-center gap-2 px-5 py-2 rounded-full text-xs font-black uppercase tracking-widest mb-5" style={{ background: 'rgba(212,97,122,0.08)', border: '1.5px solid #F9C5CC', color: '#D4617A' }}>
                ...
                <p className="mt-4 max-w-xl mx-auto text-base" style={{ color: '#7A3545' }}>Compassionate mentors who understand student life — here to listen, guide, and walk alongside you.</p>
              </div>
              <div className="relative flex items-center justify-center gap-6">
                <button onClick={() => goMentor(-1)} className="absolute left-0 z-20 w-11 h-11 rounded-2xl flex items-center justify-center transition-all"
                  style={{ background: 'rgba(212,97,122,0.10)', border: '1.5px solid #F4A0B0', color: '#D4617A' }}
                  onMouseEnter={e => e.currentTarget.style.background = 'rgba(212,97,122,0.22)'} onMouseLeave={e => e.currentTarget.style.background = 'rgba(212,97,122,0.10)'}>
                  <ChevronLeft size={20} />
                </button>
                <div className="flex items-center justify-center gap-5 w-full overflow-hidden px-14">
                  {mentorVisible.map((mIdx, pos) => {
                    ...
                    return (
                      <motion.div key={`${mIdx}-${pos}`} layout animate={{ scale: isActive ? 1 : 0.85, opacity: isActive ? 1 : 0.45 }}
                        ...
                        style={{ width: isActive ? '270px' : '210px', background: 'rgba(255,255,255,0.65)', border: isActive ? '2px solid #F4A0B0' : '1.5px solid #FFE8ED', backdropFilter: 'blur(16px)', boxShadow: isActive ? '0 20px 60px rgba(212,97,122,0.15), 0 4px 16px rgba(0,0,0,0.06)' : 'none' }}>
                        ...
                        {isActive && (
                          <div className="absolute top-5 right-5 flex items-center gap-1 px-2.5 py-1 rounded-full" style={{ background: 'rgba(254,240,138,0.5)', border: '1px solid rgba(234,179,8,0.35)' }}>
                            <Star size={11} className="text-yellow-500 fill-yellow-500" />
                            <span className="text-yellow-700 text-xs font-black">{mentor.rating}</span>
                          </div>
                        )}
                        <h3 className="font-black text-base leading-tight mb-1" style={{ color: '#3D1520' }}>{mentor.name}</h3>
                        ...
                        {isActive && (
                          <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="mt-4 pt-4 flex items-center justify-between" style={{ borderTop: '1.5px solid #FFE8ED' }}>
                            <span className="text-xs" style={{ color: '#7A3545', opacity: 0.55 }}>{mentor.experience}</span>
                            <span className="text-xs font-black px-3 py-1.5 rounded-xl" style={{ background: 'rgba(212,97,122,0.10)', color: '#D4617A' }}>View Profile →</span>
                          </motion.div>
                        )}
                      </motion.div>
                    );
                ...
                <button onClick={() => goMentor(1)} className="absolute right-0 z-20 w-11 h-11 rounded-2xl flex items-center justify-center transition-all"
                  style={{ background: 'rgba(212,97,122,0.10)', border: '1.5px solid #F4A0B0', color: '#D4617A' }}
                  onMouseEnter={e => e.currentTarget.style.background = 'rgba(212,97,122,0.22)'} onMouseLeave={e => e.currentTarget.style.background = 'rgba(212,97,122,0.10)'}>
              ...
              <div className="flex justify-center gap-2 mt-8">
                {MENTORS.map((_, i) => (
                  <button key={i} onClick={() => setMentorIdx(i)} className="rounded-full transition-all duration-300"
                    style={{ width: i === mentorIdx ? 24 : 7, height: 7, background: i === mentorIdx ? 'linear-gradient(90deg, #D4617A, #C44A6A)' : '#F9C5CC' }} />
                ))}
              </div>
              <div className="text-center mt-10">
                <button onClick={() => navigate('/mentor')} className="px-10 py-4 font-black text-white rounded-2xl transition-all text-base"
        ...
        <section id="faq" className="py-20 px-6">
          <FadeIn>
            <div className="max-w-4xl mx-auto">
              <div className="text-center mb-14">
                <h2 className="text-4xl md:text-5xl font-black mb-3 tracking-tight" style={{ color: '#3D1520' }}>
                  Got Questions? <span style={{ background: 'linear-gradient(135deg, #D4617A, #C44A6A)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>We've Got Answers.</span>
                </h2>
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  {FAQ_DATA.map((cat, i) => (
                    <button key={i} onClick={() => { setActiveCategory(i); setOpenFaq(null); }} className="px-6 py-2.5 rounded-full font-bold text-sm transition-all"
                      style={activeCategory === i ? { background: 'linear-gradient(135deg, #D4617A, #C44A6A)', color: 'white', boxShadow: '0 4px 20px rgba(212,97,122,0.30)' } : { background: 'rgba(255,255,255,0.60)', border: '1.5px solid #F9C5CC', color: '#7A3545' }}>
                      {cat.title}
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-4">
                {FAQ_DATA[activeCategory].faqs.map((f, i) => (
                  <GlassAccordion key={i} question={f.question} answer={f.answer} isOpen={openFaq === i} onClick={() => setOpenFaq(openFaq === i ? null : i)} />
                ))}
              </div>
            </div>
          </FadeIn>
        ...
        <section className="py-16 px-6 pb-28">
          <FadeIn>
            <div className="max-w-5xl mx-auto relative rounded-[3rem] overflow-hidden text-center"
              style={{ background: 'linear-gradient(135deg, #C44A6A 0%, #D4617A 40%, #b83060 100%)', boxShadow: '0 40px 100px rgba(196,74,106,0.45), 0 12px 40px rgba(0,0,0,0.15)' }}>
              {[0, 1].map(j => (
                <motion.div key={j} animate={{ rotate: j === 0 ? 360 : -360 }} transition={{ duration: j === 0 ? 26 : 34, repeat: Infinity, ease: 'linear' }}
                  className="absolute pointer-events-none rounded-full"
                  style={{ width: j === 0 ? 380 : 260, height: j === 0 ? 380 : 260, top: j === 0 ? '-110px' : 'auto', right: j === 0 ? '-110px' : 'auto', bottom: j === 1 ? '-80px' : 'auto', left: j === 1 ? '-80px' : 'auto', border: `${j === 0 ? 48 : 32}px solid rgba(255,255,255,0.055)` }} />
              ))}
              <div className="relative z-10 px-8 py-20">
                ...
                <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.65, delay: 0.1 }}
                  className="text-4xl md:text-6xl font-black text-white mb-5 leading-tight tracking-tight">
                  Your Healing{' '}
                  <span style={{ background: 'linear-gradient(135deg, #F9C5CC, #FFE8ED)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Starts Here.</span>
                </motion.h2>
                <motion.p initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }}
                  className="text-lg mb-10 max-w-xl mx-auto font-light leading-relaxed" style={{ color: 'rgba(255,255,255,0.85)' }}>
                ...
                <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }} className="flex flex-wrap justify-center gap-5">
                  <button onClick={() => navigate('/appointment')} className="px-12 py-5 font-black text-[#C44A6A] text-lg rounded-2xl transition-all active:scale-95"
                  ...
                  <button onClick={() => navigate('/chatbot')} className="px-10 py-5 font-bold text-white text-lg rounded-2xl transition-all active:scale-95"
...
const FeatureCard = ({ icon, title, desc, link, accent, light }: any) => {
  const navigate = useNavigate();
  return (
    <motion.div whileHover={{ y: -10, scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => navigate(link)}
      ...
      onMouseEnter={e => { e.currentTarget.style.boxShadow = `0 20px 60px rgba(212,97,122,0.15), 0 4px 20px rgba(0,0,0,0.06)`; e.currentTarget.style.borderColor = accent + '80'; e.currentTarget.style.background = 'rgba(255,255,255,0.85)'; }}
      onMouseLeave={e => { e.currentTarget.style.boxShadow = 'none'; e.currentTarget.style.borderColor = '#F9C5CC'; e.currentTarget.style.background = 'rgba(255,255,255,0.60)'; }}>
      <div className="absolute bottom-0 left-0 right-0 h-1 rounded-b-[2.5rem] opacity-0 group-hover:opacity-100 transition-opacity duration-400 pointer-events-none" style={{ background: `linear-gradient(90deg, ${accent}, #F4A0B0, ${accent})` }} />
      <div className="absolute -top-8 -left-8 w-24 h-24 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" style={{ background: `radial-gradient(circle, ${light} 0%, transparent 70%)`, filter: 'blur(12px)' }} />
      <div className="relative z-10">
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-7 transition-transform duration-300 group-hover:scale-110" style={{ background: light, border: `1.5px solid ${accent}25`, color: accent }}>{icon}</div>
        ...
        <div className="flex items-center gap-2 font-black text-sm group-hover:gap-3 transition-all" style={{ color: accent }}>Explore <ArrowRight size={16} /></div>
      </div>
    </motion.div>
  );
};
...
const ServiceTile = ({ icon, title, link, accent }: any) => {
  const navigate = useNavigate();
  return (
    <motion.div whileHover={{ y: -8, scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => navigate(link)}
      ...
      onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.90)'; e.currentTarget.style.borderColor = accent + '60'; e.currentTarget.style.boxShadow = `0 14px 40px rgba(0,0,0,0.08), 0 0 20px ${accent}25`; }}
      onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.65)'; e.currentTarget.style.borderColor = '#F9C5CC'; e.currentTarget.style.boxShadow = 'none'; }}>
      <div className="w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-4 transition-transform duration-300 group-hover:scale-125" style={{ background: accent + '15', color: accent }}>{icon}</div>
...
const GlassAccordion = ({ question, answer, isOpen, onClick }: any) => (
  <div className="rounded-[2rem] overflow-hidden transition-all"
    style={{ background: isOpen ? 'rgba(212,97,122,0.06)' : 'rgba(255,255,255,0.60)', border: `1.5px solid ${isOpen ? '#F4A0B0' : '#F9C5CC'}`, backdropFilter: 'blur(12px)' }}>
    <button onClick={onClick} className="w-full p-6 flex items-center justify-between text-left font-bold text-base" style={{ color: '#3D1520' }}>
      <span>{question}</span>
      <ChevronDown size={20} className="shrink-0 ml-4 transition-transform duration-400" style={{ color: '#D4617A', transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)' }} />
    </button>
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }} className="overflow-hidden">
          <div className="px-6 pb-6 text-sm leading-relaxed font-medium pt-0" style={{ color: '#7A3545', borderTop: '1.5px solid #FFE8ED' }}>
